...
function renderCart() {
  ...
  cart.forEach((item, idx) => {
    ...
    div.innerHTML = `
      <img src="${item.img || item.image}" alt="${item.product || item.name}" style="width:100%;height:120px;object-fit:cover;border-radius:14px;">
      <div class="product-card-details">
        <div class="product-title">${item.product || item.name}</div>
  ...
  cartTotal.textContent = `Total Quantity: ${totalQty} | Total Amount: GH¢${total}`;
